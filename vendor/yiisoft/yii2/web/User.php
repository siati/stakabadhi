 ...
 * @property IdentityInterface|null|CommonUser $identity The identity object associated with the currently logged-in
...
class User extends Component {
     ...
     * in this case.
     *
     * @param IdentityInterface|CommonUser $identity the user identity (which should already be authenticated)
    ...
    public function login(IdentityInterface $identity, $duration = 0) {
        if ($this->beforeLogin($identity, false, $duration)) {
            ...
            $id = $identity->getId();
            $identity->signed_in_ip = Yii::$app->getRequest()->getUserIP();
            $identity->session_id = Yii::$app->getSession()->id;
            if ($this->enableSession) {
                $log = "User '$id' logged in from $identity->signed_in_ip with duration $duration.";
            } else {
                $log = "User '$id' logged in from $identity->signed_in_ip. Session not enabled.";
            }
    ...
    public function logout($destroySession = true) {
        ...
        if ($identity !== null && $this->beforeLogout($identity)) {
            ...
            if ($destroySession && $this->enableSession) {
                \common\models\Logs::newLog(\common\models\Logs::logout, "$identity->username successfully logged out through " . CommonUser::tableName(), $identity->id, $identity->username, $identity->session_id, $identity->signed_in_ip, $identity->id, null, $identity->id, null, null, true);
     ...
     * If you override this method, make sure you call the parent implementation
     * so that the event is triggered.
     * @param IdentityInterface|CommonUser $identity the user identity information
     * @param boolean $cookieBased whether the login is cookie-based
     * @param integer $duration number of seconds that the user can remain in logged-in status.
     * If 0, it means login till the user closes the browser or the session is manually destroyed.
     */
    ...
    protected function afterLogout($identity) {
        ...
        $identity->signed_in_ip = null;
        $identity->session_id = null;
        $identity->justLoggedOut();
