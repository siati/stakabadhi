...
class AccessRule extends Component {
    ...
    public function allows($action, $user, $request) {
        if ($this->allowBool($action, $user, $request)) {
    ...
    /**
     * Check whether the Web user needs to log in to the application
     * @param Action $action the action to be performed
     * @param User $user the user object
     * @param Request $request
     * @return boolean true - user needs to be authenticated
     */
    public function loginRequired($action, $user, $request) {
        if ($this->allowBool($action, $user, $request) && !empty($this->roles))
     ...
     * @return boolean|null true if the user is allowed, false if the user is denied, null if the rule does not apply to the user
     */
    protected function allowBool($action, $user, $request) {
        return $this->matchAction($action) && $this->matchRole($user) && $this->matchIP($request->getUserIP()) && $this->matchVerb($request->getMethod()) && $this->matchController($action->controller) && $this->matchCustom($action);
    }
